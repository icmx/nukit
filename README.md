...
### Date
  ...
  - [**isValid**](src/date/isValid/index.ts) — returns true if value is valid Date or suits for Date creation.
  - [**ts**](src/date/ts/index.ts) — returns a Unix numeric timestamp for current or specified date
  - [**absoluteBetween**](src/date/absoluteBetween/index.ts) — returns total amount of time between two dates (years, months, days, hours etc). All units are independent to each other
